...
namespace ASRuntime.operators
{
	class OpLogic
	{
		...
		{
			var testvalue = step.arg1.getValue(scope, frame);

			if (testvalue.rtType > RunTimeDataType.unknown && frame.player.swc.getClassByRunTimeDataType(testvalue.rtType).isLink_System) //只有linksystem才可能有隐式类型转换
			{
				//***将对象转bool***
				BlockCallBackBase cb_obj_tobool = frame.player.blockCallBackPool.create();
				cb_obj_tobool.scope = scope;
				cb_obj_tobool.step = step;
				object[] a = cb_obj_tobool.cacheObjects; //new object[2];
				a[0] = frame;
				a[1] = scope;
				cb_obj_tobool.step = step;
        ...
        private static void _cb_obj_tobool(BlockCallBackBase sender, object args)
		{
			object[] a = (object[])sender.args;
			StackFrame frame = (StackFrame)a[0];
			RunTimeScope scope = (RunTimeScope)a[1];
			var v = TypeConverter.ConvertToBoolean(frame._tempSlot1.getValue(), frame, sender.step.token); //frame._tempSlot1.getValue();
