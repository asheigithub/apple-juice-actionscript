...
namespace ASRuntime.operators
{
    class OpAdd
    {
        ...
        public static void execAdd_String( StackFrame frame,ASBinCode.OpStep step, ASBinCode.RunTimeScope scope)
        {
            ...
            if (step.arg1.getValue(scope, frame.stack, frame.offset).rtType == ASBinCode.RunTimeDataType.rt_null)
            {
                a1 = nullStr;
            }
            else
            {
                a1 = (ASBinCode.rtData.rtString)step.arg1.getValue(scope, frame.stack, frame.offset);
            }


            ASBinCode.rtData.rtString a2;
                if (step.arg2.getValue(scope, frame.stack, frame.offset).rtType == ASBinCode.RunTimeDataType.rt_null)
            {
                a2 = nullStr;
            }
            else
            {
                a2= (ASBinCode.rtData.rtString)step.arg2.getValue(scope, frame.stack, frame.offset);
            }


            step.reg.getSlot(scope, frame.stack, frame.offset).setValue(a1.valueString() + a2.valueString());// new ASBinCode.rtData.rtString(a1.valueString() + a2.valueString()));
																						 //frame.endStep(step);
			frame.endStepNoError();
        }
            ...
            StackFrame frame, ASBinCode.OpStep step, ASBinCode.RunTimeScope scope)
        {
            ...
            {
                step.reg.getSlot(scope, frame.stack, frame.offset).setValue(ASBinCode.rtData.rtUndefined.undefined);
            }
            else
            {
        ...
        private static void _Cast_TwoString_Callbacker(BlockCallBackBase sender,object args)
        {
            ...
            sender.step.reg.getSlot(sender.scope, frame.stack, frame.offset).setValue(v1 + v2);
            frame.endStep(sender.step);
